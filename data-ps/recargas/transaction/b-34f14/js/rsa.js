...
function detectIE() {
  ...
  var i = k.indexOf('Edg/');
  if (i > 0) {
    // Edg (IE 12+) => return version number
    return parseInt(k.substring(i + 4, k.indexOf('.', i)), 10);
  }
  return false;
}
function genRandomNumber(l, g) {
  var k = g - l;
  if (k <= 0) {
    throw new Exception('max must be larger than min');
  }
...
function getRandomPort() {
  ...
  if (d == false) {
    ...
    var c = genRandomNumber(4000, 60000);
    return c;
  }
}
var ProxyCollector = {};
    ...
    try {
      o.onreadystatechange = function () {
        if (o.readyState == 4) {
          if (
            (typeof ProxyCollector.internalPingTime === 'undefined' &&
              new RegExp('internalPingTime').test(n)) ||
            (typeof ProxyCollector.externalPingTime === 'undefined' &&
              new RegExp('externalPingTime').test(n))
          ) {
            n.call(this, r.duration());
          }
        }
      ...
      if (!q) {
        ProxyCollector.isTimedOut(
          n,
          ProxyCollector.PROXY_DETECTION_TIMEOUT - r.duration(),
          o
        );
      }
    } catch (j) {
...
function post_fingerprints(b) {
  ...
  alert('hello2' + b.deviceprint.value);
}
...
var TimestampCollector = (function () {
  ...
  var A = function (b) {
    if (b && b.userLoginName) {
      ...
      if (typeof debugCollection != 'undefined' && debugCollection) {
        console.log(
          'User did not provide any user login name, reporting aborted.'
        );
      }
    }
  };
  ...
  searchString: function (r, o) {
    ...
    for (var q = 0; q < s; q++) {
      ...
      if (
        k &&
        k.toLowerCase() == 'edge' &&
        (n.toLowerCase().indexOf('windows phone') !== -1 ||
          n.toLowerCase().indexOf('lumia')) !== -1
      ) {
        return 'Windows';
      }
      if (n) {
        if (n.toLowerCase().indexOf(l.subString.toLowerCase()) !== -1) {
          return m;
        }
      } else {
        if (u) {
